...
const char *nombreRegion(int region) {
    switch (region) {
        case 1: return "GBA";
        case 2: return "PAMPEANA";
        case 3: return "NOROESTE";
        case 4: return "NORESTE";
        case 5: return "CUYO";
        case 6: return "PATAGONIA";
...
void resolucionPunto1() {
    ...
    if (archivo == NULL) { printf("No se pudo abrir el archivo.\n"); return; }
    if (fgets(linea, MAX_LINEA, archivo) == NULL) { fclose(archivo); return; }
    if (!obtenerColumnas(linea, nombresColumnasPunto1, indicesColumnasPunto1, 6)) { fclose(archivo); return; }

    inicializarRegiones(regiones);

    while (fgets(linea, MAX_LINEA, archivo) != NULL) {
        token = strtok(linea, " \t\r\n;");
        indice = 0; region = -1; whog = 0.0; wper = 0.0;
...
void resolucionPunto2() {
    ...
    if (fgets(linea, MAX_LINEA, archivo) == NULL) { fclose(archivo); return; }
    if (!obtenerColumnas(linea, nombresColumnasPunto2, indicesColumnasPunto2, 9)) { fclose(archivo); return; }
    ...
    while (fgets(linea, MAX_LINEA, archivo) != NULL) {
        token = strtok(linea, " \t\r\n;");
        indice = 0; id = -1; whog = 0.0; wper = 0.0; region = -1; sexoSel = -1; edadSel = -1;
        tpOcupacionAutoconsumo = -1; tpTrabajoTotal = -1; tpTnr = -1;
...
/*** ESTE ERA EL PUNTO 3 Y 4 VIEJO QUE NO SE MOSTRABA, ARRIBA EL MODIFICADO***/
//void Puntos3y4(const char* ruta_archivo) {
//    FILE *archivo = fopen(ruta_archivo, "r");
//    char linea[MAX_LINEA];
//    int idx_region = -1, idx_whog = -1, idx_dctotal = -1, idx_dcporedad = -1;
//
//    float p3_hogares[4][CANT_REGIONES] = {0};
//    int p4_regs_totales[2][CANT_REGIONES] = {0};
//    float p4_hog_totales[2][CANT_REGIONES] = {0};
...
//            while (*p != '\0' && *p != ';' && *p != '\n' && *p != '\r') campo[idx++] = *p++;
//            campo[idx] = '\0';
//            limpiarComillasLocales(campo);
//
//            if (strcmp(campo, "REGION") == 0) idx_region = colActual;
//            else if (strcmp(campo, "WHOG") == 0) idx_whog = colActual;
//            else if (strcmp(campo, "TIPO_HOGAR_DCTOTAL") == 0) idx_dctotal = colActual;
//            else if (strcmp(campo, "TIPO_HOGAR_DCPOREDAD") == 0) idx_dcporedad = colActual;
//
//            if (*p == ';') p++;
//            colActual++;
//        }
...
//        char *p = linea;
//        int colActual = 0;
//        int region = -1, dctotal = -1, dcporedad = -1;
//        float whog = 0.0;
//        int es_na_poredad = 0;
//
//        while (*p != '\0') {
...
//            limpiarComillasLocales(campo);
//
//            if (colActual == idx_region && campo[0] != '\0') region = atoi(campo);
//            else if (colActual == idx_whog && campo[0] != '\0') whog = (float)atof(campo);
//            else if (colActual == idx_dctotal && campo[0] != '\0') dctotal = atoi(campo);
//            else if (colActual == idx_dcporedad) {
//                if (campo[0] == '\0' || strcmp(campo, "NA") == 0) es_na_poredad = 1;
//                else dcporedad = atoi(campo);
//            }
//
//            if (*p == ';') p++;
//            colActual++;
...
//    for (r = 0; r < CANT_REGIONES; r++) {
//        for (d = 0; d < 2; d++) {
//            if (p4_regs_totales[d][r] > 0) printf("%-8d %-20d %-15d %-15.0f\n", r + 1, d, p4_regs_totales[d][r], p4_hog_totales[d][r]);
//        }
//    }
//
//    printf("\n%-8s %-15s %-12s %-12s %-22s %-12s %-10s\n", "REGION", "DCPOREDAD", "registros", "hogares", "Edad", "Totales_dem", "Prop");
...
void resolucionPunto5() {
    ...
    while (fgets(linea, MAX_LINEA, archivo) != NULL) {
        token = strtok(linea, " \t\r\n;");
        indice = 0; whog = 0.0; region = -1; tipoHogarDcTotal = -1; cuidadoSoloHogar = -1;
...
void procesarPuntoTrabajo(const char *nombreVariableFiltro, int *pVarFiltroCol, FilaReporte *tablaDestino, int cantFilasDestino) {
    ...
    *pVarFiltroCol = -1;
    ...
    while (fgets(linea, MAX_LINEA, archivo) != NULL) {
        token = strtok(linea, " \t\r\n;");
        indice = 0; wper = 0.0; variableFiltro = -1; tpAutoconsumo = -1; tpTrabajoTotal = -1; tpTnr = -1;
...
void extraerCampo(char **linea, char *buffer) {
    char *aux = *linea;
    char *dest = buffer;
    while(*aux != ';' && *aux !='\n' && *aux!='\r' && *aux!='\0') {
        if(*aux != '"') {
...
int buscarColumna(char *linea_header, char *nombreColumna) {
    char campo[80];
    char *linea = linea_header;
    int posicion = 1;
    while(*linea != '\0' && *linea !='\n' && *linea != '\r') {
        extraerCampo(&linea, campo);
        if(strcmp(campo, nombreColumna) == 0) return posicion;
        if(*linea == ';') linea++;
        posicion++;
    }
    return -1;
...
void saltarCampos(char **linea, int n) {
    char *aux = *linea;
    for(int i=0; i < n; i++) {
        while(*aux != ';' && *aux !='\n' && *aux!='\r' && *aux!='\0') aux++;
        if(*aux == ';') aux++;
...
void parsearLinea (char *linea, int *posCol, tRegistroOrig *reg) {
    char *aux = linea;
    char buffer[80];
    int posActual = 1, campoIndice, saltos;
    int *destinos[14];

    *destinos       = &reg->id;
    *(destinos + 1)  = &reg->whog;
    *(destinos + 2)  = &reg->wper;
    *(destinos + 3)  = &reg->region;
    *(destinos + 4)  = &reg->sexo;
    *(destinos + 5)  = &reg->nivelEducativo;
    *(destinos + 6)  = reg->tcs;
    *(destinos + 7)  = (reg->tcs + 1);
    *(destinos + 8)  = (reg->tcs + 2);
    *(destinos + 9)  = (reg->tcs + 3);
    *(destinos + 10) = reg->tp;
    *(destinos + 11) = (reg->tp + 1);
    *(destinos + 12) = (reg->tp + 2);
    *(destinos + 13) = (reg->tp + 3);

    for(campoIndice = 0; campoIndice < 14 ; campoIndice++) {
        saltos = *(posCol + campoIndice) - posActual;
        saltarCampos(&aux, saltos);
        extraerCampo(&aux, buffer);
        *(*(destinos + campoIndice)) = atoi(buffer);
        posActual = *(posCol + campoIndice) + 1;
        if (*aux == ';') aux++;
    }
}
...
void punto_8 (const char *nombreArchOrigen, const char *nombreArchDest) {
    ...
    if(!archOrig || !archDest) return;

    char linea[10000];
    tRegistroOrig reg;
    int posCol[14];
    const char *nombres[14] = {"ID", "WHOG", "WPER", "REGION", "SEXO_SEL", "NIVEL_EDUCATIVO_AGRUPADO", "TCS_GRANGRUPO_TRABAJOTOTAL", "TCS_GRANGRUPO_OCUPACIONYAUTOCONSUMO", "TCS_GRANGRUPO_TNR", "TCS_GRANGRUPO_PERSONALES", "TP_GRANGRUPO_TRABAJOTOTAL", "TP_GRANGRUPO_OCUPACIONYAUTOCONSUMO", "TP_GRANGRUPO_TNR", "TP_GRANGRUPO_PERSONALES"};

    fgets(linea, sizeof(linea), archOrig);
    for(int i = 0; i < 14; i++) *(posCol + i) = buscarColumna(linea, (char *)*(nombres + i));

    while(fgets(linea, sizeof(linea), archOrig)) {
        parsearLinea(linea, posCol, &reg);
...
void intercambiarGen(void *a, void *b, size_t tamElem) {
    ...
    memcpy(aux,a,tamElem); memcpy(a,b,tamElem); memcpy(b,aux,tamElem);
    free(aux);
}
...
bool CrearVector(Vector* v, size_t tamElem, size_t cap) {
    ...
    v->cap=cap; v->tamElem=tamElem; v->ce=0;
    return true;
}
...
bool redimensionar(Vector* v, size_t NuevaCap) {
    ...
    v->vec=nVec; v->cap=NuevaCap;
    return true;
}

void DestruirVector(Vector* v) { free(v->vec); }
...
int Punto_9(Vector* v) {
    ...
    while(i < ult) {
        ...
        total_ponderado=0; total_wper=0;
        ...
        if(total_wper>0) {
            ...
            horas = promedio/60; minutos = promedio%60;
        }
...
int Punto_10(Vector* v) {
    ...
    while(i < ult) {
        strcpy(tipo_trabajo,((tRegModif*)i)->tipo_trabajo);
        total_ponderado_mujer=0; total_wper_mujer=0;
        total_ponderado_hombre=0; total_wper_hombre=0;
        promedio_hombre=0; promedio_mujer=0;
    ...
    while(i < ult) {
        ...
        while(i < ult && strcmp(tipo_trabajo,((tRegModif*)i)->tipo_trabajo)==0) {
            ...
            while(i < ult && strcmp(tipo_trabajo,((tRegModif*)i)->tipo_trabajo)==0 && sexo == ((tRegModif*)i)->sexo_sel) {
                ...
                total_ponderado=0; total_wper=0; promedio=0;
